...
void PathProducer::process(juce::Rectangle<float> fftBounds, double sampleRate)
{
	...
	while (leftChannelFifo->getNumCompleteBuffersAvailable() > 0)
	{
		if (leftChannelFifo->getAudioBuffer(tempIncomingBuffer))
		{
			...
			juce::FloatVectorOperations::copy(monoBuffer.getWritePointer(0, 0), monoBuffer.getWritePointer(0, size), monoBuffer.getNumSamples() - size);
	...
	/*
	* if ther are FFT data buffers to pull
...
void ResponseCurveComponent::timerCallback()
{
	...
	auto samepleRate = audioProcessor.getSampleRate();

	leftPathProducer.process(fftBounds, samepleRate);
	rightPathProducer.process(fftBounds, samepleRate);
...
SimpleEQAudioProcessorEditor::SimpleEQAudioProcessorEditor(SimpleEQAudioProcessor& p)
	...
	analyzerEnableButtonAttachment(audioProcessor.apvts, "Analyzer Enabled", analyzerEnabledButton)
{
